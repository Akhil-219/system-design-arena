import { createDesign, deleteDesign, getDesignById, getMyDesigns, updateDesign } from "../services/desgin.service.js";
...
const createDesignController= asyncHandler(async(req,res)=>{
    ...
    const {design}= await createDesign({problemId, userId})
    return res
    .status(201)
...
const updateDesignController = asyncHandler(async(req, res)=>{
    ...
    const userId=req.user._id
    const {design,version}= await updateDesign({designId, userId,diagramData,notes})
    return res.status(200).json(new ApiResponse(200, {design, version}, "Design updated successfully"))
})
