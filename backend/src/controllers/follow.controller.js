...
import { followUser, getFollowers, getFollowing, getFollowStatus, unfollowUser } from "../services/follow.service.js"
...
const getFollowStatusController=asyncHandler(async(req, res)=>{
    const {username} =req.params
    const followStatus= await getFollowStatus({username, currentUserId:req.user._id})
    let message=`You are following ${username}`
    if(!followStatus){
        message=`You are not following ${username}`
    }
    return res.status(200).json(new ApiResponse(200,{isFollowing:followStatus},message)) 
})

export {unfollowUserController, followUserController, getFollowersController, getFollowingController,getFollowStatusController }