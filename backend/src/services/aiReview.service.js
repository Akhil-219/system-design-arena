...
const problem = await Problem.findById(
  ...
  const prompt = buildReviewPromptV1({
    problem,
    version,
  });
  const aiRespons= await generateAiReview(prompt)
  ...
  requirementCoverage:
    aiResponse.requirementCoverage,
  ...
  missedConcepts:
    aiResponse.missedConcepts,

  componentFeedback:
    aiResponse.componentFeedback,

  improvements:
    aiResponse.improvements,

  conceptualDepth:
    aiResponse.conceptualDepth,

  tradeoffAwareness:
    aiResponse.tradeoffAwareness,

  productionReadiness:
    aiResponse.productionReadiness,
